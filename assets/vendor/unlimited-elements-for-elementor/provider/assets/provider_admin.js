...
this.setPathSelectImages=function(path,basePath,baseUrl){if(basePath)
g_dataProviderUC.pathSelectImagesBase=basePath;if(baseUrl)
g_dataProviderUC.urlSelectImagesBase=baseUrl;if(path)
...
function initEditor_tinyMCE_GetOptions(objEditorsOptions){if(jQuery.isEmptyObject(objEditorsOptions))
return{};for(var key in objEditorsOptions){var options=objEditorsOptions[key];if(options!==null)
return options;}
return{};}
...
function initEditors_afterTimeout(objSettings,arrEditorNames,isForce){if(typeof window.tinyMCEPreInit==="undefined"&&arrEditorNames.length)
throw new Error("Init "+arrEditorNames[0]+" editor error. no other editors found on page");var idPrefix=objSettings.getIDPrefix();jQuery(arrEditorNames).each(function(index,name){var inputID=idPrefix+name;inputID=inputID.replace("#","");if(isForce===true||window.tinyMCEPreInit.mceInit.hasOwnProperty(inputID)===false||window.tinyMCEPreInit.mceInit[inputID]===null)
...
this.initEditors=function(objSettings,isForce){var arrEditorNames=objSettings.getFieldNamesByType("editor_tinymce");if(arrEditorNames.length===0)
return;setTimeout(function(){initEditors_afterTimeout(objSettings,arrEditorNames,isForce);});}
this.destroyEditors=function(objSettings){var arrEditorNames=objSettings.getFieldNamesByType("editor_tinymce");if(arrEditorNames.length===0)
return;var idPrefix=objSettings.getIDPrefix();jQuery.each(arrEditorNames,function(index,name){var editorID=idPrefix+name;editorID=editorID.replace("#","");var objEditor=tinyMCE.EditorManager.get(editorID);if(objEditor){try{if(tinymce.majorVersion==="4")
