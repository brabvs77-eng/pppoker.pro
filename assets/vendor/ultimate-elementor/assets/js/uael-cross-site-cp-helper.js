'use strict';window.XsUtils=window.XsUtils||(function(){function extend(object,defaultObject){var result=defaultObject||{};var key;for(key in object){if(object.hasOwnProperty(key)){result[key]=object[key];}}
return result;}
return{extend:extend};})();window.xsLocalStorage=window.xsLocalStorage||(function(){var MESSAGE_NAMESPACE='cross-domain-local-message-uae';var options={iframeId:'cross-domain-iframe-uae',iframeUrl:undefined,initCallback:function(){}};var requestId=-1;var iframe;var requests={};var wasInit=false;var iframeReady=true;function applyCallback(data){if(requests[data.id]){requests[data.id](data);delete requests[data.id];}}
...
function init(customOptions){options=XsUtils.extend(customOptions,options);var temp=document.createElement('div');if(window.addEventListener){window.addEventListener('message',receiveMessage,false);}else{window.attachEvent('onmessage',receiveMessage);}
...
function isApiReady(){if(!wasInit){return false;}
if(!iframeReady){return false;}
return true;}
function isDomReady(){return(document.readyState==='complete');}
return{init:function(customOptions){if(!customOptions.iframeUrl){throw'Please specify the iframe URL';}
if(wasInit){return;}
