...
Object.defineProperty(exports,'__esModule',{value:true});};})();return __webpack_require__(714);})();});$(document).ready(function(){if($('.uael-particle-yes').length){Object.keys(window.scope_array).forEach(function(i){$scope=window.scope_array[i];particles_background_script($scope);});}});function particles_background_script($scope){id=$scope.data('id');element_type=$scope.data('element_type');particles_style=$scope.find('.uael-particle-wrapper').data('uael-partstyle');if(typeof particles_style!='undefined'){if(element_type=='column'){$column=$scope.find('.elementor-widget-wrap .elementor-background-overlay');$particle_wrap=$scope.find('.uael-particle-wrapper');if($column.next('#uael-particle-'+id).length==0){$column.after($particle_wrap);}}else{$bg_wrap=$scope.find('.elementor-element-overlay ~ .elementor-background-overlay');$particle_wrap=$scope.find('#uael-particle-'+id);if($bg_wrap.next('#uael-particle-'+id).length==0){$bg_wrap.after($particle_wrap);}}
$selector=$scope.find('#uael-particle-'+id);}else{particles_style=$scope.data('uael-partstyle');$selector=$scope;$content=$('<div class="uael-particle-wrapper" id="uael-particle-'+id+'"></div>');if(element_type=='column'){if($scope.find('.elementor-background-overlay').length==0||$scope.find('.elementor-column-wrap').length==0){$scope.append($content);}else if($scope.find('.elementor-background-overlay').length==0&&$scope.find('.elementor-column-wrap').length!=0){$column=$scope.find('.elementor-column-wrap');$column.after($content);}else{$column=$scope.find('.elementor-background-overlay');$column.after($content);}}else{if($scope.find('.elementor-background-overlay ~ .elementor-container').length==0){$scope.prepend($content);}else{$scope.find('.elementor-background-overlay').after($content);}}}
var particles_dot_color=$selector.data('uael-partcolor'),particles_custom=$selector.data('uael-partdata'),particles_opacity=$selector.data('uael-partopacity'),particles_direction=$selector.data('uael-partdirection');if(particles_custom!==''&&particles_style=='custom'){tsParticles.load('uael-particle-'+id,particles_custom);}else if(particles_style!=='custom'){var number_value=150,shape_type='circle',shape_nb_sides=5,opacity_value=0.6,opacity_random=true,opacity_anim_enable=false,line_linked=false,move_speed=4,move_random=true,size_value=2,size_random=true,size_anim_enable=false,onhover='repulse',move_direction='none',interactive=false,image_value=false;if('default'==particles_style){line_linked=true;opacity_random=false;move_random=false;move_speed=6;}else if('nasa'==particles_style){number_value=160;shape_type='circle';opacity_value=1;opacity_anim_enable=true;move_speed=1;size_value=3;onhover='bubble';}else if('snow'==particles_style){opacity_value=0.5;size_value=4;move_speed=3;move_direction=particles_direction;number_value=200;opacity_random=false;}else if('flow'==particles_style){number_value=14;shape_type='polygon';shape_nb_sides=6;opacity_value=0.3;move_speed=5;size_value=40;size_random=false;size_anim_enable=true;}else if('bubble'==particles_style){move_speed=5;move_direction='top';number_value=500;size_value=1;size_random=false;opacity_value=0.6;opacity_random=false;}else if('snowflakes'==particles_style){number_value=40;shape_type='image';image_value={"src":uael_particles_script.snowflakes_image,"replaceColor":true};opacity_value=0.6;size_value=23;opacity_random=false;opacity_anim_enable=false;move_direction=particles_direction;move_speed=2;size_random=true;}else if('christmas'==particles_style){number_value=25;shape_type='images';image_value=[{src:uael_particles_script.gift,width:300,height:300},{src:uael_particles_script.tree,width:300,height:300},];opacity_value=0.9;size_value=25;opacity_random=false;opacity_anim_enable=false;move_direction=particles_direction;move_speed=4;size_random=true;}else if('halloween'==particles_style){number_value=30;shape_type='images';image_value=[{src:uael_particles_script.skull,width:300,height:300},{src:uael_particles_script.moon,width:300,height:300},{src:uael_particles_script.pumpkin,width:300,height:300},{src:uael_particles_script.ghost,width:300,height:300},{src:uael_particles_script.bat,width:300,height:300},];opacity_value=0.8;size_value=20;opacity_random=false;opacity_anim_enable=false;move_speed=4;size_random=true;move_direction='none';}
...
if($scope.hasClass('uael-particle-adv-yes')){var particles_number=$selector.data('uael-partnum');var particles_size=$selector.data('uael-partsize');var particles_speed=$selector.data('uael-partspeed');var particles_interactive=$selector.data('uael-interactive');if(particles_number!=''){number_value=particles_number;}
...
tsParticles.load('uael-particle-'+id,config);}}})(jQuery);