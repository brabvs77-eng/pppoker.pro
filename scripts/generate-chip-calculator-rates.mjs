...
const localeCurrency = {
  ...
  tj: { currency: 'TJS', intlLocale: 'tg-TJ', symbol: 'сомонӣ' },
};

function roundRate(value, currency) {
  if (currency === 'USD') return 1;
  if (currency === 'RUB') return Math.round(value);
  if (currency === 'KZT' || currency === 'AMD' || currency === 'TJS') return Math.round(value);
...
async function main() {
  ...
  console.log(
    `Updated chip-calculator rates (${config.ratesUpdatedAt}): RUB ${roundRate(config.rates.RUB, 'RUB')}, AMD ${roundRate(config.rates.AMD, 'AMD')}, UZS ${roundRate(config.rates.UZS, 'UZS')}, KZT ${roundRate(config.rates.KZT, 'KZT')}, TJS ${roundRate(config.rates.TJS, 'TJS')}`,
  );
}
