...
GitHub Actions on `main` (after build + smoke) `rsync`s `apps/web/out/` → `/var/www/pppokerpro/data/www/pppoker.pro` as `pppokerpro`. Required secret: `DEPLOY_SSH_KEY`. Optional: `DEPLOY_HOST`, `DEPLOY_USER`, `DEPLOY_PORT`, `DEPLOY_PATH`.
